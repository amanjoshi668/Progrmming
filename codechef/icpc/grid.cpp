...
bool fun(vector<string> &s, string pat){
    bool found = true;
    REP(0,4){
        bool is=true;
        REPV(j,0,4)if(!(s[j][i]==pat[j] or (s[j][i]>='0' and s[i][j]<='9')))is=false;
        if(is)found = true;
        IREPV(j,3,0)if(!(s[j][i]==pat[j] or (s[j][i]>='0' and s[i][j]<='9')))is=false;
        if(!is)found = false;
    }
    return found;
...
int main(int argc, char*argv[]){
    ...
    while(t--){
        ...
        TRV(s)a.pb(it);
                ...
                string res="gridsnotpossible";
                bool result=false;
               // cout<<"hrerere"<<endl;;
        REP(0,(1<<8)){
            if(__builtin_popcountll(i)==4){
                vector<string> use;
                vector<string> not_use;
                REPV(j,0,8){
                    if(CHECK_BIT(i,j)){
                        use.pb(a[j]);
                    }
                ...
                REPV(j,0,16){
                    ...
                    REPV(k,0,16){
                        ...
                        bool ans=true;
                        REPV(l,0,4)ans&=fun(mat, not_use[l]);
                        if(ans){
                            result=true;
                            string temp = "";
                            REPV(l,0,4)temp+=use[l];
                            if(temp<res)res=temp;
                        }
                    }
                    next_permutation(all(use));
                    //cout<<use<<endl;;
                }
            }
        }
        REP(0,res.length())if(res[i]>='0' and res[i]<='9')res[i]='A';
            REP(0,16){
                cout<<res[i];
                if(i%4==3)cout<<endl;
            }
