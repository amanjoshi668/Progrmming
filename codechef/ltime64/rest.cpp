...
int main(int argc, char*argv[]){
    ...
    while(t--){
        ...
        vll a(n);
        cin>>a;
        a.push_back({LLONG_MAX,LLONG_MAX});
        sort(all(a));
        lo index=0;
        vll query(m);
        REP(0,m){
        ...
        for(auto it:query){
            lo no=it.X;
            lo in=it.Y;
            //debug3(index,no,a[index]);
            if(index==n){
                assert(a[index-1].Y<=no);
                ans[in]=-1;
                continue;
            }
            while(a[index].Y<=no){
            ...
            if(index==n)ans[in]=-1;
            else ans[in]=max(0LL,a[index].X-no);
            //debug3(it,a[index],ans[in]);
        }
        TRV(ans)cout<<it<<endl;
    }
    return 0;
}
