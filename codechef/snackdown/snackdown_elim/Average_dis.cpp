...
int main(int argc, char*argv[]){
    ...
    while(t--){
        ...
        vl r_dis(n+m+2,0);
        vl l_dis(n+m+2,0);
        REPE(0,n,0,m)if(a[i][j]=='1')r_dis[i+j]++;
        debug(r_dis);
        REP(0,n){
            vl dd = r_dis;
            REPV(j,0,m){
                ...
                REPV(k,i,n)if(a[k][j]=='1'){
                    r_dis[k-i]--;
                    if(k!=i)l_dis[k-i]++;
                }
                REPV(k,0,n+m+1)r_dis[k] = r_dis[k+1];
                IREPV(k,n+m,1)l_dis[k]=l_dis[k-1];
                debug2(i,j);
                debug3(r_dis,l_dis,distance);
            }
            cout<<endl;
            REPV(j,0,m)if(a[i][j]=='1')dd[j]--;
            REPV(k,0,n+m+1)dd[k] = dd[k+1];
            r_dis=dd;
            fill(all(l_dis),0LL);
        }      
        REP(1,n+m-1)cout<<distance[i]<<" ";
        cout<<endl;
    }
    return 0;
}
