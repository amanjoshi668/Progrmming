...
lo dp[64][64][64][2];
lo fun(lo a, lo b, lo pos, string &s, bool carry){
    // debug4(a,b,pos,carry);
    if(a==0 and b==0 and pos == s.length() and !carry)return 1;
    if(a<0 or b<0 or pos>=s.length())return 0;
    if(dp[a][b][pos][carry]!=-1)return dp[a][b][pos][carry];
    lo res=0;
    if(!carry){
        if(s[pos]=='0'){
            res+=fun(a-1, b-1,pos+1, s, true);
            res+=fun(a,b,pos+1, s, false);
        }
        else{
            res+=fun(a-1,b,pos+1,s,false);
            res+=fun(a,b-1,pos+1,s, false);
        }
    }
    else{
        if(s[pos]=='0'){
            res+=fun(a-1,b,pos+1, s,true);
            res+=fun(a,b-1,pos+1, s,true);
        }
        else{
            res+=fun(a-1,b-1,pos+1,s, true);
            res+=fun(a,b,pos+1,s, false);
    ...
    dp[a][b][pos][carry]=res;
    return res;
}
lo brute(lo a,lo b, lo c){
    lo res=0;
    lo ca = __builtin_popcountll(a);
    lo cb = __builtin_popcountll(b);
    REPE(0,c+1,0,c+1){
        if(i+j==c){
            lo x = i;
            lo y = j;
            if(__builtin_popcountll(i) == ca and __builtin_popcountll(j) == cb){
               debug2(i,j);
                res++;
            }
        }
