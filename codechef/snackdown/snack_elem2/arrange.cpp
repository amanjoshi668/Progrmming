...
int main(int argc, char*argv[]){
    ...
    while(t--){
        ...
        cin>>a;
        lo index=0;
        while(a[index]=='#' and index<n)index++;
        lo hash = 0;
        lo dot = 0;
        lo sum = 0;
        lo ans=0;
        ...
        REP(index+1,n){
            if(a[i]=='.' and hash){
                ...
                if(dot==hash){
                    // debug("herer");
                    // debug4(hash, sum,index, i);
                    ans+=2*hash*(i)-sum-hash*index-(hash*(hash+1))/2-hash*hash*2;
                    index += hash;
                    hash=dot=sum=0;
                }
            }
            else if(a[i]=='#' and index!=i+1){
                sum+=i;
                hash++;
            }
            else if(a[i]=='#' and index==i-1){
            ...
            // debug4(i,hash, dot, index);
        }
        if(hash>0)cout<<"-1"<<endl;
        else cout<<ans<<endl;
    }
    return 0;
}
