...
vl fun1(lo n){
	...
	bool x = false;
	REP(0,n){
		cout<<1<<" "<<i+1<<" "<<(i+1)%n+1<<" "<<(i+2)%n+1<<endl;
		cin>>a[i];
	}
	vl b(n);
	REP(0,n-1)b[i] = a[i]^a[i+1];
	b.back() = a[0]^a.back();
	lo one = 0;
	vl ans(n);
	lo i = n-1;
...
vl fun2(lo n, lo offset = 0){
	vl a(n);
	bool x = false;
	REP(0,n){
		cout<<1<<" "<<i+1+offset<<" "<<(i+1)%n+1+offset<<" "<<(i+2)%n+1+offset<<endl;
		cin>>a[i];
	}
	vl b(n);
	REP(0,n-1)b[i] = a[i]^a[i+1];
...
int main(int argc, char *argv[])
{
	...
	while(t--){
		...
		vl ans;
		if(n%3==1)ans = fun1(n);
		else if(n%3==2)ans = fun2(n);
		else{
			lo x = n/2;
			if(x%3==0)x-=2;
			if(x%3==2)x--;
			ans = fun1(x);
			auto temp = fun2(n-x,x);
			TRV(temp)ans.pb(it);
		}
		cout<<2<<" ";
		REP(0,n)cout<<ans[i]<<" ";
