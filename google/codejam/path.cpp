...
lo intlog(ld base, ld x) {
    return (lo)(log(x) / log(base) + 1);
}
...
int main(int argc, char*argv[]){
    ...
    REPV(t,1,T+1){
        ...
        lx=ly=mx=my=0;
        REP(0,a.length()){
            // assert(lx==mx and ly==my and a[i]==b[i]);//throw 1;
            if(a[i]=='S')ly++;
            else lx++;
            if(b[i]=='S')my++;
            else mx++;
        }
        // cout<<lx<<" "<<ly<<" "<<mx<<" "<<my<<" "<<n-1<<endl;
        // assert(mx!=my and mx!=n-1);//cout<<"ERROR "<<mx<<" "<<my<<endl;
        cout<<"Case #"<<t<<": ";
        cout<<b;
        cout<<endl;
    }
