...
struct Node{
    lo count;
    map<lo, pair<vll, Node*>> M;
};
Node* newnode(vll factor, lo count){
    Node* res = new Node();
    res->count = count;
    return res;
}
Node* root;
void insert(Node *node, vll factor, lo count){
    if(!node->M.count(factor[0].first))
        node->M[factor.back().first] = {factor, newnode(factor, count)};
    else{
        auto child = node->M[factor.back().first].second;
        auto factor_child = node->M[factor.back().first].first;
        factor_child.pop_back();
        child->M[factor_child.back().first] = {factor_child, newnode(factor_child, child->count)};
        factor[factor.size() - 1].second -= factor_child.back().second;
        insert(child, factor, count);
        node->count = 0;
    }
}
