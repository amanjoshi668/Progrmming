...
bigint distance(pair<bigint, bigint> P1, pair<bigint, bigint> P2)
{
    bigint res = P1.first - P2.first;
    res = res.abs();
    bigint other = P1.second - P2.second;
    other = other.abs();
    return res + other;
}
int main(int argc, char *argv[])
{
    ...
    REP(1, 300)
    {
        bigint nx = points.back().first * ax + bx;
        bigint ny = points.back().second * ay + by;
    ...
    REP(0, 200)
    {
        bigint dis = distance({sx, sy}, points[0]);
        lo cnt = 0;
        lo j = i;
        while (dis <= t)
        {
            cnt++;
            dis += distance(points[j], points[j + 1]);
            j++;
        }
        res = max(res, cnt);
        cnt = 0;
        j = i;
        dis = distance({sx, sy}, points[j]);
        while (dis <= t and j > 0)
        {
            cnt++;
            dis += distance(points[j], points[j - 1]);
