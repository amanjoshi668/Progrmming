...
void solve()
{
    ...
    lo p = *max_element(all(lens));
    for (auto len1 = 0; len1 + a <= p; len1++)
    {
        auto len2 = p - a - len1;
        if (len1 >= 2 * b or (len1 < a and len1 >= b))
            continue;
        if (len2 >= 2 * b or (len2 < a and len2 >= b))
            continue;
        if ((lens.size() - 1 + (len1 >= a) + (len2 >= a)) % 2 == 0)
        {
            alice();
            return;
        }
