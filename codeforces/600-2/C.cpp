...
int main(int argc, char *argv[])
{
    ...
    REPV(t, 1, T+1){
        ...
        REPV(mask, 0, 1<<9){
            if(popcount(mask)>1){
                REPV(b, 0, 9)if(CHECK_BIT(mask, b)){
                    auto nmask = mask ^ (1<<b);
                    lo x = numbers[b].X;
                    lo y = numbers[b].Y;
                    bool amodd = ((x+y)%2 == 1);
                    REPE(0, 11, 0, 11){
                        if(dp[nmask][i][j]){
                            bool odd = dp[nmask][i][j] == 2;
                            if(odd and amodd){
                                dp[mask][(x+j)%11][(y+i)%11] = 1;
                            }
                            else if(odd){
                                dp[mask][(x+j)%11][(y+i)%11] = 2;
                                dp[mask][(x+i)%11][(y+j)%11] = 2;
                            }
                            else if(amodd){
                                dp[mask][(x+j)%11][(y+i)%11] = 2;
                                dp[mask][(x+i)%11][(y+j)%11] = 2;
