...
int main(int argc, char*argv[]){
    ...
    while(t--){
        string a,b,c;
        cin>>a>>b>>c;
        lo x1 = stoll(a.substr(0,a.find('/')));
        lo x2 = stoll(a.substr(a.find('/')+1));
        lo y1 = stoll(b.substr(0,b.find('/')));
        lo y2 = stoll(b.substr(b.find('/')+1));
        lo z1 = stoll(c.substr(0,c.find('/')));
        lo z2 = stoll(c.substr(c.find('/')+1));
        lo z=(x2*y2)/gcd(x2,y2);
        z=(z*z2)/gcd(z,z2);
        lo ans=x1*(z/x2)+y1*(z/y2)+z1*(z/z2);
        lo k = gcd(ans,z);
        cout<<ans/k<<"/"<<z/k<<endl;
    }
    return 0;
