...
int main(int argc, char *argv[])
{
    ...
    REPE(0, n + 1, 0, 2)
    REPV(k, 0, count + 1) dp[i][j][k] = INT_MAX;
    if (a[0] == 0)
    {
        dp[0][0][1] = 0;
        dp[0][1][0] = 0;
    ...
    REP(1, n)
    {
        REPV(c, 0, count + 1)
        {
            if (a[i])
            {
                if (a[i] % 2 == 0)
                {
                    dp[i][0][c] = min(dp[i - 1][0][c], dp[i - 1][1][c] + 1);
                }
                else
                {
                    dp[i][1][c] = min(dp[i - 1][1][c], dp[i - 1][0][c] + 1);
                }
            }
            else
            {
                if (c > 0)
                    dp[i][0][c] = min(dp[i - 1][0][c - 1], dp[i - 1][1][c - 1] + 1);
                dp[i][1][c] = min(dp[i - 1][0][c] + 1, dp[i - 1][1][c]);
            }
        }
    }
    cout << min(dp[n - 1][0][count], dp[n - 1][1][count]) << endl;
    return 0;
}
