...
bool cmp(ordered_set &A, ordered_set &B){
    return A.size() < B.size();
}
bool valid = true;
ordered_set dfs_checker(lo node, lo par)
{
    vector<ordered_set> V;
    TRV(g[node])
    if (it != par)
        {
            V.push_back(dfs_checker(it, node));
        }
    sort(all(V), cmp);
    ordered_set S ;
    if(!V.empty()){
    ...
    if (S.order_of_key(a[node]) != c[node]){
        // debug(node);
        // debug3(a[node], S.order_of_key(a[node]) , c[node])
        valid = false;
    }
    S.insert(a[node]);
    return S;
