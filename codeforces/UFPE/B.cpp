...
int main(int argc, char *argv[])
{
    ...
    REP(a,b+1){
        ...
        lo prime_in_range = 0;
        debug2(i, factor);
        for(lo iik=0;iik<(1<<l);iik++){
            lo d = 0 ;
            lo n = 1;
            for(lo j=0;j<l;j++)if(CHECK_BIT(iik,j)){
                d++;
                n*=factor[j];
            }
            if(d%2==0)prime_in_range+=(d+n-1)/n-(c-1+n-1)/n;
            else prime_in_range-=(d+n-1)/n-(c-1+n-1)/n;
        }
        debug(prime_in_range);
        proccessed[num] = (d-c+1) - prime_in_range;
        ans+=(d-c+1)-prime_in_range;
        debug2(i,ans);
    }
    cout<<ans<<endl;
    return 0;
