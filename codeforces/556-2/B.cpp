...
void fun(lo x, lo y){
    ...
    REP(0, 4){
        auto nx = x + dx[i];
        auto ny = y + dy[i];
        if(!correct(nx, ny, n, n))ans = false;
        else {
            if(a[nx][ny] == '#')ans = false;
            else if(visited[nx][ny]) ans = false;
            visited[nx][ny] = true;
        }
    }
    if(!ans){
...
int main(int argc, char*argv[]){
    ...
    REPE(0, n, 0, n)if(!visited[i][j] and a[i][j] == '.')fun(i, j);
    bool ans = true;
    REPE(0, n, 0, n)if(a[i][j] == '.' and !visited[i][j])ans = false;
