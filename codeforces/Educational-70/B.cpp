...
vl fun(lo i, lo j, lo k){
    ...
    while(!Q.empty() and counter < 1000){
        auto top = Q.front();
        Q.pop();
        if(first and top.first == k)first = false;
        else if(res[top.first]==-1)res[top.first] = top.second;
        auto tt = (top.first+i)%10;
        if(res[tt] == -1)Q.push({tt, top.second+1});
        tt = (top.first + j)%10;
        if(res[tt] == -1)Q.push({tt, top.second+1});
        counter++;
    }
...
int main(int argc, char *argv[])
{
    ...
    lo ans[10][10];
    vl dig[10][10][10];
    REPE(0, 10,0, 10)REPV(des, 0, 10)REPV(start, 0, 10){
        dig[i][j][start] = fun(i, j, start);
    } 
    // reverse(all(a));
    ...
    for(int i = 0;i<10; i++)for(int j = i; j<10; j++){
        ...
        for(int c = 0; c<a.length()-1; c++){
            ...
            else cur += (dig[i][j][s][d] - 1);
            if(i == 6 and j==8)debug4(cur, dig[i][j][s][d], s, d);
        }
        if(!found)ans[i][j] = ans[j][i] = cur;
    }
    for(int i = 0; i<10; i++){
