...
vector<lo> valid(1001, true);
vector<ll> edges;
vector<lo> graph[1001];
lo root;
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        if (graph[root].size() == 1)
        {
            lo other = -1;
            REP(1, n + 1)
            if (deg[i] == 1 and root != -1)
                other = i;
            if (other != -1)
            {
                cout << "? " << other << " " << root << endl;
                cin >> root;
            }
            cout << "! " << root << endl;
            break;
        }
        cout << "? " << graph[root][0] << " " << graph[root][1] << endl;
        lo other_root = 0;
        cin >> other_root;
        if (other_root == root)
        {
            dfs(graph[root][0], root);
