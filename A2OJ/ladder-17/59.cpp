...
vvl g(INF, vl(26,-1));
vl win(INF,0),loose(INF,0);
lo size_trie=0;
void add_trie(string s){
    lo node = 0;
    REP(0,s.length()){
        lo c = s[i]-'a';
        if(g[node][c] == -1)g[node][c] = ++size_trie;
        node = g[node][c];
    }
}
void answer(bool x){
...
int main(int argc, char*argv[]){
    ...
    dfs(0);
    if(k==1)answer(win[0]);
    else if(!win[0])answer(win[0]);
    else if(loose[0])answer(win[0]);
    else if(k%2==1)answer(win[0]);
    else answer(!win[0]);
    return 0;
}
