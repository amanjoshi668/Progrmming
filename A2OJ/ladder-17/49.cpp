...
vl taken(INF,0),g[INF];
bool comp(ll a, ll b){
    if (a.first==b.first)
    return a.second<b.second;
    return a.first>b.first;
}
lo l,modl,jed=LLONG_MAX;
ll dfs(lo v, lo oj){
    vll wek;
    for (int i=0; i<g[v].size(); i++)
    {
        if (g[v][i]==oj)
        continue;
        wek.push_back(dfs(g[v][i], v));
        if (wek.back()==make_pair(0LL, 0LL))
        wek.pop_back();
    }
    sort(wek.begin(), wek.end(), comp);
    if (wek.empty())
    {
        if (!taken[v])
