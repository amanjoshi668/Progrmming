...
int main(int argc, char*argv[]){
    ...
    while(true){
        lo cont=0;
        lo cur = a.back();
        while(a.back()==cur and a.size()>0){
            a.pop_back();
            cont++;
        }
        if(cont%x!=0){
            cout<<pow(x,min(sum,cur));
            return 0;
        }
        else{
            cont/=x;
            REP(0,cont)a.push_back(cur+1);
        }
        //debug2(i,cont);
    }
    // if(n==98304 and x==2)debug3(res,add,sum);
    // cout<<(pow(x,res)*pow(x,add))%MOD<<endl;
    return 0;
}
