...
vll g[INF];
vl dp(INF,0),max_path(INF,0),ans(INF,0);
void dfs(lo node, lo par){
    lo to_sub=0;
    TRV(g[node])if(it.X!=par){
        dfs(it.X,node);
        dp[node]+=2*(it.Y) + dp[it.X];
        max_path[node] = max(max_path[node], max_path[it.X] + it.Y);
    }
    ans[node] = dp[node]-max_path[node];
}
