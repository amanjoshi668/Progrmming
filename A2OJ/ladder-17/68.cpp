...
int main(int argc, char*argv[]){
    ...
    REP(0,m){
        deque<int> dq;
        lo interval = min(n,(t[i]-pT)*d);
        REPV(j,1,interval+1){
            while(!dq.empty() and dp[1^p][dq.back()] <= dp[1^p][j])dq.pop_back();
            dq.push_back(j);
        }
        REPV(j,1,n+1){
            lo k = j+interval;
            if(k<=n){
                while(!dq.empty() and dp[1-p][dq.back()] <= dp[1-p][k])dq.pop_back();
                dq.push_back(k);
            }
            dp[p][j] = dp[1-p][dq[0]] + b[i]-abs(a[i]-j);
            while(!dq.empty() and dq[0]<=(j-interval))dq.pop_front();
        }
        pT = t[i];
        p^=1;
    }
