...
int main(int argc, char*argv[]){
    ...
    cin>>a;
    vl cnt(a.length()+1,0);
    vl hash1_string(a.length()+1,0);
    vl power_cal(a.length()+1,1);
    lo seed = 1331;
    string bad;
    cin>>bad;
    REP(1,a.length()+1){
        cnt[i]=cnt[i-1];
        if(bad[a[i-1]-'a']=='0')cnt[i]++;
        power_cal[i] = power_cal[i-1]*seed;
        // p2 = (p2*131)%MOD2;
        hash1_string[i] = (hash1_string[i-1] * seed) + a[i-1];
        // hash2_string[i] = (hash2_string[i-1]+(p2*(a[i-1]-'0'))%MOD2)%MOD2;
    }
    set<lo>proccessed;
    lo ans=0;
    lo k;
    cin>>k;
    // debug2(hash1_string,hash2_string);
    REPE(1,a.length()+1,i,a.length()+1){
        lo str1 = (hash1_string[j]-hash1_string[i-1]*power_cal[j-i+1]);
        // lo str2 = (((hash2_string[j]-hash2_string[i-1]+MOD2)%MOD2)*power(power(131,i-1,MOD2),MOD2-2,MOD2))%MOD2;
        if(present(proccessed,str1))continue;
        // debug4(i-1,j-1,str1,str2);
        proccessed.insert(str1);
        if(cnt[j]-cnt[i-1] <= k)ans++;
    }
    cout<<ans;
    return 0;
}
