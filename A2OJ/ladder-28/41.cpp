...
vl a,b;
vl ans;
void dfs(lo node, lo par, bool flip1, bool flip2, lo depth){
    debug5(node, par, flip1, flip2, depth);
    if(depth&1){
        if(flip1){
            a[node]^=1;
        }
    ...
    TRV(g[node])if(it!=par){
        dfs(it, node, flip1, flip2, depth+1);
    }
}
