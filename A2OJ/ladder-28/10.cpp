...
int main(int argc, char *argv[])
{
    ...
    lo n;
    cin>>n;
    vector<string> rat, child, man, captain;
    while(n--){
        string x,y;
        cin>>x>>y;
        if(y=="rat")rat.pb(x);
        else if(y=="woman" or y=="child")child.pb(x);
        else if(y=="man")man.pb(x);
        else captain.pb(x);
    }
    TRV(rat)cout<<it<<endl;
    TRV(child)cout<<it<<endl;
    TRV(man)cout<<it<<endl;
    TRV(captain)cout<<it<<endl;
    return 0;
}
