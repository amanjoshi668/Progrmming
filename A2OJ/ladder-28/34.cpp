...
int main(int argc, char *argv[])
{
    ...
    while(t--){
        string s;
        getline(cin,s);
        bool freda = false, rain = false;
        if(s.find("miao.")==0)rain = true;
        reverse(all(s));
        if(s.find(".alal")==0)freda = true;
        if(freda and !rain)cout<<"Freda's"<<endl;
        else if(!freda and rain)cout<<"Rainbow's"<<endl;
