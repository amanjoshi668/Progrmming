...
vl seg_tree(4*INF,0);
lo update(lo t,lo node, lo l, lo r, lo index, lo val){
    if(index<l or index>r)return seg_tree[node];
    if(l==r)return seg_tree[node] = val;
    lo mid = (l+r)/2;
    update(t^1, node*2+1, l, mid, index, val);
    update(t^1, 2*node+2, mid+1, r, index, val);
    if(!t)seg_tree[node] = seg_tree[node*2+1] ^ seg_tree[node*2+2];
    else seg_tree[node] = seg_tree[node*2+1] | seg_tree[node*2+2];
...
int main(int argc, char *argv[])
{
    ...
    // REP(0,14)cout<<seg_tree[i]<<" ";
    // cout<<endl;
    REP(0,m){
        lo a,b;
        cin>>a>>b;
        a--;
        cout<<update(n&1, 0,0, (1<<n)-1, a, b)<<endl;
    }
    return 0;
}
