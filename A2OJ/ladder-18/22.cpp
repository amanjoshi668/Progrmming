...
lo find_index(vl &a, lo t, lo start){
    lo r = a.size();
    lo l = start;
    while(r>l+1){
        lo mid = (l+r)/2;
        if(a[mid]-a[start]>=t)r = mid;
        else l = mid;
    }
    return r;
}
int main(int argc, char *argv[])
{
    ...
    REPV(t,1,n+1){
        ...
        lo cur = 0;
        while(cur<n){
            lo next1 = find_index(sum1, t, cur);
            lo next2 = find_index(sum2, t, cur);
            if(next1<next2){
                w1++;
                cur = next1;
            }
        ...
        debug3(t,w1,w2);
        if(a[n-1] == 1){
            if(w1>w2)ans.pb(mp(w1,t));
        }
        else{
            if(w2>w1)ans.pb(mp(w2,t));
        }
    }
    sort(all(ans));
    cout<<ans.size()<<endl;
    TRV(ans)cout<<it.X<<" "<<it.Y<<endl;
    return 0;
}
