...
int main(int argc, char *argv[])
{
    ...
    REP(0,n){
        ...
        while(!Q.empty()){
            lo node = Q.front();
            Q.pop();
            TRV(g[node])if(dis[i][it] == -1){
                dis[i][it] = dis[i][node] + 1;
                Q.push(it);
            }
        }
    }
    ...
    REPV(xxx,0,2){
        swap(s[0],t[0]);
        REPE(0,n,0,n){
            lo dis1 = dis[s[0]][i] + dis[i][j] + dis[j][t[0]];
            lo dis2 = dis[s[1]][i] + dis[i][j] + dis[j][t[1]];
            if(dis1 <= l[0] and dis2 <= l[1]){
                ans = min(ans, dis1 + dis2 - dis[i][j]);
            }
        }
    }
    if(dis[s[0]][t[0]] <= l[0] and dis[s[1]][t[1]] <= l[1]){
