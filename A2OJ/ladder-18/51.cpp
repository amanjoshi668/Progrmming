...
bitset<INF> used;
vl p;
inline void fun(lo node){
    for(lo i = node;!used[i]; i=p[i]){
        used[i] = true;
    }
}
int main(int argc, char *argv[])
{
    ...
    if(k<m){
        for(lo i=0;i<n and k<m ; i++){
            vl pos(n,-1);
            lo cur = 0;
            for(lo j=  p[i]; j!=i; j = p[j])pos[j] = cur++;
            pos[i] = cur;
            cur = 0;
            for(lo j=i+1; j<n and k<m; j++){
                if(pos[j]>=cur){
                    cout<<i+1<<" "<<j+1<<" ";
                    k++;
                    cur = pos[j]+1;
