...
struct seg_tree_implicit{
    lo m,d;
    seg_tree_implicit *L,*R;
    seg_tree_implicit(lo _m):m(_m), d(0), L(0), R(0){}
};

void push(seg_tree_implicit *T){
    T->m = max(T->m, T->d);
    if(T->L){
        T->L->d = max(T->L->d, T->d);
    }
...
lo get(seg_tree_implicit *T, lo tl, lo tr, lo i){
    push(T);
    lo tm = (tl + tr) >> 1;
    if(T->L and i<tm){
        return max(get(T->L, tl, tm, i), T->m);
    }
    if(T->R and i >= tm){
        return max(get(T->R, tm, tr, i), T->m);
    }
    return T->m;
}

void upd(seg_tree_implicit *T, lo tl, lo tr, lo l, lo r, lo d){
    push(T);
    if(r<=tl or l>=tr){
        return ;
    }
    ...
    if(!T->L){
        T->L = new seg_tree_implicit (T->m);
        T->R = new seg_tree_implicit (T->m);
    }
    lo tm = (tl + tr) >> 1;
    upd(T->L, tl, tm, l, r, d);
    upd(T->R, tm, tr, l, r, d);
...
int main(int argc, char *argv[])
{
    ...
    lo q;
    cin>>q;
    auto tv = new seg_tree_implicit(0);
    auto th = new seg_tree_implicit(0);
    while(q--){
        lo l, r;
        char c;
        cin>>l>>r>>c;
        if(c=='U'){
            lo res = get(tv, 1, n+1, l);
            cout << r - res << endl;
            upd(th, 1, n+1, res+1, r+1, l);
            upd(tv, 1, n+1, l, l+1, r);
        }
        else{
            lo res = get(th, 1, n+1, r);
            cout << l - res << endl;
            upd(tv, 1, n+1, res+1, l+1, r);
            upd(th, 1, n+1, r, r+1, l);
        }
    }
    return 0;
}
