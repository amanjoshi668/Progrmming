...
int main(int argc, char *argv[])
{
    ...
    sort(all(a));
    a.push_back({h,w});
    auto comb = [fact, inv_fact](lo n, lo k){
        if(n<0 or k<0)return 0LL;
        derr2(n,k);
        assert(n>=0 and k>=0);
        lo res = fact[n+k];
        res = mul(res, inv_fact[k]);
        res = mul(res, inv_fact[n]);
        return res;
    };
    vl dp(n+1,0);
    REP(0,n+1){
        dp[i] = comb(a[i].X - 1, a[i].Y - 1);
        REPV(j,0,i){
            sub(dp[i], mul(dp[j],comb(a[i].X - a[j].X, a[i].Y-a[j].Y)));
        }
        dp[i]%=MOD;
    }
    cout<<dp[n];
