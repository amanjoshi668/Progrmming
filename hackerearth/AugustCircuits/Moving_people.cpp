...
int main(int argc, char*argv[]){
    ...
    int ru, rd, cf, cl;
    int cur_ru, cur_rd, cur_cf, cur_cl;
    ru = cf = cur_ru = cur_cf = 0;
    rd = cur_rd = R-1;
    cl = cur_cl = C-1;
    while(q--){
        int choice;
        ...
        if(choice == 1){
            ...
            if(cf > cl or cf >= C or cf < 0 or ru > rd or ru >= R or rd < 0){
                cout<<0<<endl;
            }
            else{
                auto ans = dp[rd][cl];
                if(cf)ans -= dp[rd][cf - 1];
                if(ru)ans -= dp[ru - 1][cl];
                if(cf and ru)ans += dp[ru-1][cf-1];
                cout<<ans<<endl;
            }
        }
        debug4(ru, cf, rd, cl);
        debug4(cur_ru, cur_cf, cur_rd, cur_cl);
    }
    return 0;
}
