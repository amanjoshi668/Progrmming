...
int main(int argc, char*argv[]){
    ...
    vl skill(m+1);
    REP(1, m+1)cin>>skill;
    vl iindex = {2, 3, 5, 7, 11, 13, 17, 19, 23};
    while(iindex.back() > m)iindex.pop_back();
    ...
    REP(0, n){
        ...
        TRV(iindex){
            lo count = 0;
            for(int j = it; j<a.length(); j+=it)if(a[i]=='1')count++;
            if(count>1)valid = false;
        }
        if(valid)players.push_back(a);
    }
    for(lo mask = 0; mask<(1<<iindex.size()); mask++){
