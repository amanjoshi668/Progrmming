    ...
    void update(lo node, lo start, lo end, lo l, lo r, lo val)
    {
        if (this->islazy)
        {
            if (lazy[node] != 0)
            {
                ...
                if (lazy[node] == 1)
                {
                    seg_tree[node] = (end - start + 1); // Update it
                    if (start != end)
                    {
                ...
                else if (lazy[node] == 2)
                {
                    ...
                    if (start != end)
                    {
                        lazy[node * 2 + 1] = 2; // Mark child as lazy
                        lazy[node * 2 + 2] = 2; // Mark child as lazy
                    }
                    lazy[node] = 0; // Reset it
                }
                else if (lazy[node] == 3)
                {
                    ...
                    if (start != end)
                    {
                        lazy[node * 2 + 1] = 3; // Mark child as lazy
                        lazy[node * 2 + 2] = 3; // Mark child as lazy
                    }
                    lazy[node] = 0; // Reset it
                }
            }
...
int main(int argc, char *argv[])
{
    ...
    lo n, q;
    cin >> n >> q;
    vector<SEGMENT_TREE> bits(25, SEGMENT_TREE(n + 2));
    REP(0, 25)bits[i].reset();
    while (q--)
    {
        lo c;
        cin >> c;
