...
int main(int argc, char *argv[])
{
    ...
    REPE(0, n, 0, n){
        ...
        lo v = order[j];
        if(!is_ancestor(v, u)){  
            dp[u][v] = depth[v] - depth[u];          
            continue;
        }
        dp[u][v] = LLONG_MAX;
        for(auto d = 0; d<=max_depth[u]; d++){
            dp[u][v] = min(dp[u][v], fun(depth[u] - depth[v]+d) + d);
            lo dis = depth[u] + d;
            if(dis%2==0 and dis){
                dis = depth[u]/2 - d/2;
                lo other = u;
                for (int i = l; i >= 0; --i)
                    if (dis&(1<<i))
                        other = up[other][i];
                dp[u][v] = min(dp[u][v], d + dp[other][v] + 1);
            }
        }
