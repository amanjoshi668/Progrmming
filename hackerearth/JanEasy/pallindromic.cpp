...
bool fun(lo l, lo r, string &s, vvl &dp){
    if(dp[l][r] != -1)
        return dp[l][r];
    if(l > r)
        return true;
    ...
    REP(r+1, s.length())
        if(s[i] == s[l])
            return true;
    int res = true;
    res = res and fun(l+1, r, s, dp);
    res = res and fun(l, r-1, s, dp);
    dp[l][r] = res ^ 1;
    debug3(l, r, dp[l][r]);
    return dp[l][r];
}
int main(int argc, char *argv[])
{
    ...
    derr2(sl, sr);
    if(sl < sr){
        a = s.substr(0, sl + 1) + s.substr(sr, s.length() - sr);
    }
    debug(a);
    vvl dp(a.length(), vl (a.length(), -1));
    auto res = fun(0, a.length() - 1, a, dp);
    if(res)
        cout<<"Alice"<<endl;
    else
    {
