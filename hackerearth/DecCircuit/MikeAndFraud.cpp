...
int main(int argc, char *argv[])
{
    ...
    if (temp_k > 1)
        factors.push_back({temp_k, 1});
    vl a(n);
    cin >> a;
    vl count(factors.size(), 0);
    lo l = 0;
    lo r = 0;
    lo ans = 0;
    lo distinct = 0;
    ...
    {
        debug3(l, r, distinct);
        while (distinct < factors.size() and r < n)
        {
            REP(0, factors.size())
            {
                // debug2(i, r);
                lo c = 0;
                lo num = a[r];
                while (num % factors[i].first == 0)
                {
                    num /= factors[i].first;
                    c++;
                }
                if (count[i] < factors[i].Y and count[i] + c >= factors[i].Y)
                    distinct++;
                // debug3(factors[i], count[i], c);
                count[i] += c;
                // debug(distinct);
            }
            r++;
        }
        while (distinct == factors.size())
        {
            ans += n - r + 1;
            debug3(l, r, ans);
            REP(0, factors.size())
