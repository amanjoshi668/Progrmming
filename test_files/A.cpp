...
int main(int argc, char *argv[])
{
    ...
    REPE(1, n + 1, 1, n + 1)
    {
        if (x[i - 1] == y[j - 1])
        {
            if (i-1 != n-j and( dp[i - 1][j - 1].second.empty() or dp[i - 1][j - 1].second.size() > 1 or (dp[i - 1][j - 1].second.size() == 1 and *dp[i - 1][j - 1].second.begin() != x[i - 1])))
            {
        debug3(i, j, x[i-1]);
                dp[i][j] = dp[i - 1][j - 1];
                dp[i][j].first++;
            ...
            else if (dp[i - 1][j - 1].second.size() == 1)
            {
                dp[i][j] = dp[i - 1][j];
                ...
                {
                    if (!dp[i][j - 1].second.empty())
                        dp[i][j].second.insert(*dp[i][j - 1].second.begin());
                    else
                    {
                        dp[i][j].second.clear();
                    }
